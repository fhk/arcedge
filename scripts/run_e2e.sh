...
#   + shapely (pip install highspy numpy pyarrow shapely).
set -euo pipefail
cd "$(dirname "$0")/.."
...
echo "=== [3/9] small instance: solve + HiGHS validation ==="
mkdir -p data
...
echo "=== [4/9] medium instance: solve + HiGHS validation ==="
...
echo "=== [5/9] large instance (~1M arcs): certified LB/UB gap ==="
...
echo "=== [6/9] downtown SF (Overture import): solve + HiGHS validation ==="
...
echo "=== [7/9] full SF street network (~1.7M TE arcs): certified gap ==="
...
echo "=== [8/9] downtown SF hub design (POI access network) vs HiGHS MIP ==="
...
echo "=== [9/9] solution output: map to street geometry, write GeoParquet ==="
...
echo "and both solutions round-trip to GeoParquet geometry."
